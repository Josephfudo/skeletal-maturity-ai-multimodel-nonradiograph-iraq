...
"""
This is v4 of the feature creation script.

It performs the same logic as v3 (creating 25 features = 15 preds + 10 tabular),
but it uses the NEW models trained with differential learning rates
(the 'best_model_diff_lr_...' files).

It saves its output to '_v4.npy' files to create a new experiment.
"""
...
def create_oof_features():
    ...
    print("\n--- Starting OOF Feature Generation for VALIDATION set (v4 - 25 features) ---")
    ...
    for fold in range(1, NUM_FOLDS + 1):
        ...
        # 2. Load all 5 models trained for this fold (using the new _diff_lr_ models)
        models_for_this_fold = []
        for backbone in VALID_BACKBONES:
            # --- !!! KEY CHANGE: Using new model files !!! ---
            model_path = os.path.join(MODELS_DIR, f'best_model_diff_lr_fold{fold}_{backbone}.pth')
            # --- !!! END OF CHANGE !!! ---
        ...
        # 4. Combine prediction features (15) and tabular features (10)
    ...
    if not all_oof_combined_feats:
        print("\nFATAL ERROR: No OOF features were generated. Could not find any 'best_model_diff_lr_...' models.")
    ...
    np.save('oof_train_features_v4.npy', oof_train_features)
    np.save('oof_train_labels_v4.npy', oof_train_labels)
    print("Saved 'oof_train_features_v4.npy' and 'oof_train_labels_v4.npy'")
    return True # Indicate success

def create_test_features():
    """Generates features for the TEST set for final prediction."""
    print("\n--- Starting Feature Generation for TEST set (v4 - 25 features) ---")
    ...
    # 2. Load ALL 25 new trained models
    ...
    total_models = sum(len(m) for m in models_by_fold.values())
    if total_models == 0:
        print("\nFATAL ERROR: No test features were generated. Could not find any 'best_model_diff_lr_...' models.")
        return False
        
    print(f"\nLoaded {total_models} total models.")
    ...
    # 5. Combine averaged prediction features (15) and tabular features (10)
    ...
    np.save('oof_test_features_v4.npy', final_test_features)
    np.save('oof_test_labels_v4.npy', all_test_labels)
    print("Saved 'oof_test_features_v4.npy' and 'oof_test_labels_v4.npy'")
    return True

if __name__ == "__main__":
    if create_oof_features(): # Only proceed if OOF features were created
        create_test_features()
        print("\n--- Stacking feature creation (v4) complete. ---")
        print("You can now run 'train_stacking_meta_model_v4.py'")
    else:
        print("\n--- Stacking feature creation (v4) FAILED. ---")
        print("Could not find new '...diff_lr...' models. Please check your training logs.")